...
class Basic_feature_label:
    def __init__(self,poi_info,config) -> None:
        self.home_location = self.set_home_location(poi_info)
        self.work_location = self.set_work_location(poi_info)
        self.city = self.set_city(poi_info)
        self.basic_features_labels_mapping = self.get_features_labels_mapping(poi_info)
    ...
    def show_basic_feature_label(self) -> pd.DataFrame:
        ...
        #在df中添加home_location和work_location，和city列
        # df['home_location'] = self.home_location
        # df['work_location'] = self.work_location
        # df['city'] = self.city
        return df

    def set_home_location(self,poi_info):
        input = poi_info
        prompt = f"请分析这个列表:{poi_info}，结合其type字段，帮我分析哪个地点（也就是poi字段）是用户的居住地，直接给出对应的地点名称（列表中其中一项的poi字段值）;如果不能判断出用户的居住地，则直接回答 无法确认用户居住地\n"
    ...
    def set_city(self,poi_info):
        input = poi_info
        prompt = f"请分析这个列表:{poi_info}，结合其poi字段，帮我分析用户常住的城市是哪里，并直接回复我：一个城市名称;如果不能判断出用户所在城市，则直接回答 无法确认用户所在城市\n"
        return ask_LLMmodel(input,prompt)
    
    def get_features_labels_mapping(self,poi_info,template_json="Handle_csv/scenario/navigation/basic_features_labels_mapping_template.json"):
        ...
        for feature,labels in basic_features_labels_mapping.items():
            if feature == "通勤基础":
                ret_info = self.sub_classify_6(poi_info)
                for label in labels:
                    basic_features_labels_mapping[feature][label] = ret_info[label]
                continue
            for label in labels:
                basic_features_labels_mapping[feature][label] = self.classify((feature,label),poi_info)
                
        return basic_features_labels_mapping

    
    def classify(self,feature_label_tuple,poi_info):
        if feature_label_tuple == ("基础信息","城市") :
            return self.city
        elif feature_label_tuple == ("基础信息","居住地") :
            return self.home_location
        elif feature_label_tuple == ("基础信息","工作地") :
            return self.work_location

        elif feature_label_tuple == ("时间规律","出行周期偏好") :
            return self.sub_classify_1(poi_info)
        elif feature_label_tuple == ("时间规律","出行时段偏好") :
            return self.sub_classify_2(poi_info)
        elif feature_label_tuple == ("空间范围","单次出行距离") :
            return self.sub_classify_3(poi_info)
        elif feature_label_tuple == ("空间范围","活动区域") :
            return self.sub_classify_4(poi_info)
        elif feature_label_tuple == ("目的地偏好","高频目的地类型") :
            return self.sub_classify_5(poi_info)
        
        elif feature_label_tuple == ("通勤空间","通勤方向") :
            return self.sub_classify_9(poi_info)
        elif feature_label_tuple == ("工作习惯","工作时长") :
            return self.sub_classify_11(poi_info)
        elif feature_label_tuple == ("时间规律","高峰出行模式") :
            return self.sub_classify_12(poi_info)
        else:
            return ""
        
    
    def sub_classify_1(self,poi_info) -> str:
        # ("时间规律","出行周期偏好")
        poi_info_dict = json.loads(poi_info)
        week_day = []
        for info in poi_info_dict:
    ...
    def sub_classify_2(self,poi_info) -> str:
        # （"时间规律","出行时段偏好")
        poi_info_dict = json.loads(poi_info)
    ...
    def sub_classify_3(self,poi_info) -> str:
        # ("空间范围","单次出行距离")
        # poi_info_dict = json.loads(poi_info)
        # # 计算所有出行距离的平均值
        # total_distance = []
        ...
        #         if i == 0:
        #             total_distance.append(get_driving_path_distance_by_address(self.city,self.home_location,days[day][i]['poi']))
        #         elif i == len(days[day])-1:
        #             total_distance.append(get_driving_path_distance_by_address(self.city,days[day][i]['poi'],self.home_location))
        #         else:
        #             total_distance.append(get_driving_path_distance_by_address(self.city,days[day][i]['poi'],days[day][i+1]['poi']))
        ...
        return "逻辑已实现,待接入含用户出发点的埋点数据"

        
    def sub_classify_4(self,poi_info) -> str:
        # ("空间范围","出行范围")
        
        return "逻辑已实现,待接入含用户出发点的埋点数据"

    def sub_classify_5(self,poi_info) -> str:
        # ("目的地偏好","高频目的地类型")
        poi_info_dict = json.loads(poi_info)
    ...
    def sub_classify_6(self,poi_info):
        # ("通勤基础","规律性行程")
        # input = poi_info
        # prompt = f"请根据输入的用户出行信息{poi_info}，判断用户是否具有规律性行程，分以下几种情况： \
        ...
        参数:
            poi_info: POI信息列表，每个元素包含poi、start_time、end_time和type
        返回:
            包含行程规律信息的字典
        """
        # 转换为DataFrame便于处理
        df = pd.DataFrame(poi_info)
    ...
    def sub_classify_7(self,poi_info) -> str:
        # ("通勤基础","通勤时长")
        return "待实现"

    def sub_classify_8(self,poi_info) -> str:
        return "待实现"

    def sub_classify_9(self,poi_info) -> str:
        return "待实现"
    
    def sub_classify_11(self,poi_info) -> str:
        # ("工作习惯","工作时长")
        poi_info_dict = json.loads(poi_info)
        ...
        work_time_list = []
        for poi_item in poi_info_dict:
            if scaned_work_location and work_start_time != None and poi_item['poi'] != self.work_location:
    ...
    def sub_classify_12(self,poi_info) -> str:
        # ("时间规律","高峰出行模式")
        poi_info_dict = json.loads(poi_info)
        # 用util中get_hour统计每个时间段出现的次数 poi_info_dict的格式为 [{poi: "xxx", type: "xxx", start_time: "xxx", end_time: "xxx"}]
        time_interval = []
